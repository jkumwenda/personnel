@extends('layouts.app')
@section('title', 'Applications For Screening')
@section('content')
    <div class="row">
            ...
            <div class="card-body">
                <div class="table-responsive">
                    @if(count($applications) > 0)
                        <table class="table table-bordered table-striped table-hover datatable" id="applicationsTable">
                        <thead>
                        <tr>
                            <th>#</th>
                        ...
                        @foreach($applications as $application)
                            <tr data-entry-id="{{ $application->id }}">
                                ...
                                <td>
                                    <a href="{{ route('applications.detailview', $application->id) }}"
                                       class="btn btn-sm brand-bg-color">{{$application->application_status == "rejected" ? "View" : "Screen"}} <i class="fa fa-eye"></i></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
